...
- [ARCHITECTURE.md](../../ARCHITECTURE.md) — JWT, `@DenyAll` / `@RolesAllowed`, package map
- [.cursor/rules/issues-core.mdc](../rules/issues-core.mdc) — endpoint security defaults
- [.cursor/rules/issues-http-contract.mdc](../rules/issues-http-contract.mdc)
- [.cursor/rules/issues-platform.mdc](../rules/issues-platform.mdc) — no secrets in prod config
- [.cursor/rules/development-process.mdc](../rules/development-process.mdc) — remediations need feature analysis + task approval before code
...
4. **Report** — write or update a markdown report under `reports/` (see command for full-pass naming).
...
| Diff-only PR review (built-in) | Task `security-review` — use this agent for Issues-wide or teach-oriented audits |
