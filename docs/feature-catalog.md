...
| Account settings | `/account/settings` (under `/pt/` or `/en/`) | authenticated | Menu → Conta → edit name/email/**language** → save (reloads `/{locale}/account/settings` on language change) → change password (LOCAL only) → **Conectar agente** → **Tokens de API** |
