...
| Edit project | `/projects/:projectId/edit` | project owner PM, admin | Project hub → **Editar** → update fields, **Nível de segurança**, and **owner** (admin or current owner); **prefix** read-only when the project has tickets; manage **Campos personalizados (projeto)** (add/edit/disable/delete) and template custom defaults → save; SPA `projectManageGuard` + API `ADMIN`/`PROJECT_MANAGER` + `requireManage` |
