...
All routes below are canonical for both Portuguese and English. The active runtime catalog changes copy, dates, numbers, and Material labels in place; language selection never adds a locale path prefix or navigates away from the current screen.
...
| Account settings | `/account/settings` | authenticated | Menu → Conta → edit name/email/**language** → save; visible UI copy changes immediately without navigation or reload → change password (LOCAL only) → **Conectar agente** → **Tokens de API** |
...
| Project dashboard | `/project/:projectId/dashboard` | authenticated (readable project) | Hub → **Painel** → default widgets; **Editar layout** / **Concluir** (autosave on drop/remove + toast); loading / **Sem dados** / error + **Tentar novamente**; `tickets-by-day` as bar; KPI titled **Tickets por status**; recent tickets ≤20 |
...
| Ticket detail | `/ticket/:ticketIdentifier` | authenticated | Kanban or search → click active ticket → **Clonar ticket** opens global create with the source project selected; or edit built-in fields (incl. **Tipo**, rich-text **Descrição**) and **Campos personalizados** (Text CF rich text; orphan former-workflow values read-only), assign, move status (Epic warns when moving to DONE with open **Subtarefas**), delete (admin/PM), **restore deleted ticket (admin/PM)**, **Vínculos** (grouped peer/hierarchy links; add/remove; cross-project search), **Subtarefas** on Epics (progress `n/m` + **Nova subtarefa**), **Anexos** (list / upload / download / delete files), comments (type `@` to mention a project member — notifies them even if not subscribed), observe; **Histórico** merges field changes, attachment add/remove, and **linked commits** (SHA, message, author, Abrir); **Atividade** shows **Agente em nome de …** when history/comments were made via API token (`via_agent`) |
| Ticket search (simple) | `/search` | authenticated | Menu → search → enter term and optional status → **Exportar** → choose **CSV** or **JSON** to download all readable matches; or open a ticket |
| Advanced search (query language) | `/search/advanced` | authenticated | Search → **Busca avançada** → enter query (incl. `cf.<key>` predicates) → **Executar** → **Exportar** → choose **CSV** or **JSON** for the last submitted valid query; open query help for field/operator reference; dismissible **context hint** (onboarding) |
...
| Shared saved query | `/search/q/:slug` | authenticated | Open shared results → **Exportar** → choose **CSV** or **JSON**; copy link; owner **Editar**; others **Clonar** |
| Create ticket | `/tickets/new` | authenticated | Header → Novo → select project → fill form (**Tipo** Épico/História/Tarefa, rich-text **Descrição** + **Campos personalizados** from in-scope defs; Text CF rich text) → optional **fase** → create. From **Clonar ticket**: select any writable target project; its template loads first, then source title, description, category, priority, type, and compatible custom fields override it; review visible omission warnings → create a fresh ticket through the normal create action |
...
| Project list | `/projects` | project-manager, admin | Conta → **Projetos** (PM or admin) or Header **Projetos** → **Gerenciar projetos**. List: **Abrir** hub, **Fases**, **Versões**, **Editar** per row (list scope: viewable projects — member ∪ owned for non-admin; all for admin) |
...
| Edit project | `/projects/:projectId/edit` | project owner PM, admin | Project hub → **Editar** → update fields, **Nível de segurança**, and **owner** (admin or current owner); **prefix** read-only when the project has tickets; **Repositório Git** (URL, provider, branch, webhook URL/secret copy + regenerate, API ingest help); manage **Campos personalizados (projeto)** (add/edit/disable/delete) and template custom defaults → save; SPA `projectManageGuard` + API `ADMIN`/`PROJECT_MANAGER` + `requireManage` |
| Workflow list | `/workflows` | project-manager, admin | Administração → **Processos** (admin or PM) → list → Editar |
